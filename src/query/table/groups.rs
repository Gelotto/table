...
fn load_groups_by_name<'a>(
  ...
  let (min, max) = match order {
    Order::Ascending => (
      maybe_cursor
        .and_then(|group_id| Some(Bound::Exclusive((parse::<GroupID>(group_id).ok()?, PhantomData))))
...
fn load_groups_created_between<'a>(
  ...
  let (min, max) = match order {
    Order::Ascending => (
      maybe_cursor
        ...
        .or_else(|| Some(Bound::Inclusive(((start.nanos(), GroupID::MIN), PhantomData)))),
      Some(Bound::Inclusive(((stop.nanos(), GroupID::MAX), PhantomData))),
    ),
    Order::Descending => (
      Some(Bound::Inclusive(((start.nanos(), GroupID::MIN), PhantomData))),
      maybe_cursor
        .and_then(|(t, id)| {
          Some(Bound::Exclusive((
            (parse::<u64>(t).ok()?, parse::<GroupID>(id).ok()?),
  ...
  for maybe_key in GROUP_IX_CREATED_AT.keys(deps.storage, min, max, order).take(PAGE_SIZE) {
