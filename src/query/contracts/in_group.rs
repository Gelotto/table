...
pub fn in_group(
  ...
  for maybe_contract_id in IX_GROUP.prefix(group_id).keys(deps.storage, min, max, order) {
  ...
  let contracts: Vec<ContractRecord> = load_contract_records(deps.storage, &contract_ids, params.details)?;
