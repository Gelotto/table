...
pub fn related_to(
    ...
    Order::Ascending => (
      if let Some((rel_name, c_id_str)) = params.cursor {
        ...
        Some(Bound::Inclusive((
          (params.address.to_string(), "".to_string(), ContractID::MIN.to_string()),
          PhantomData,
        )))
      },
      None,
  ...
  for result in REL_ADDR_2_CONTRACT_ID.keys(deps.storage, min, max, order).take(limit) {
    ...
    if let Some(contract_rel) = memoized.get_mut(&related_contract_id) {
      ...
      memoized.insert(
        related_contract_id,
        RelatedContract {
          contract: load_one_contract_record(deps.storage, related_contract_id, params.details.clone())?,
