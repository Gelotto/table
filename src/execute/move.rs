...
use crate::{
  ...
  state::{
    ...
    CONTRACT_DYN_METADATA, CONTRACT_METADATA, CONTRACT_TAGS, INDEXED_KEYS, IX_CODE_ID, IX_CREATED_AT, IX_CREATED_BY,
    IX_REV, IX_TAG, IX_UPDATED_AT, IX_UPDATED_BY, PARTITION_SIZES, VALUES_BOOL, VALUES_STRING, VALUES_TIME,
...
fn move_standard_indices(
  ...
  IX_CREATED_BY.remove(storage, (src, meta.initiator.to_string(), contract_id));
  IX_CREATED_BY.save(storage, (dst, meta.initiator.to_string(), contract_id), &X)?;

  IX_CREATED_AT.remove(storage, (src, meta.time.nanos(), contract_id));
  IX_CREATED_AT.save(storage, (dst, meta.time.nanos(), contract_id), &X)?;

  // Update "update" metadata indices
  let up_meta = CONTRACT_DYN_METADATA.load(storage, contract_id)?;

  IX_UPDATED_BY.remove(storage, (src, up_meta.initiator.to_string(), contract_id));
  IX_UPDATED_BY.save(storage, (dst, up_meta.initiator.to_string(), contract_id), &X)?;

  IX_UPDATED_AT.remove(storage, (src, up_meta.time.nanos(), contract_id));
  IX_UPDATED_AT.save(storage, (dst, up_meta.time.nanos(), contract_id), &X)?;
